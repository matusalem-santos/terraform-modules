...
resource "aws_api_gateway_integration" "default" {
  ...
  connection_id           = var.vpc_link_enabled ? aws_api_gateway_vpc_link.vpc-link.*.id[count.index] : element(var.connection_ids, count.index)
...
resource "aws_api_gateway_deployment" "default" {
  ...
  depends_on        = [aws_api_gateway_method.default, aws_api_gateway_integration.default, aws_api_gateway_account.config ]
}
...
resource "aws_api_gateway_stage" "default" {
  count = var.deployment_enabled && var.stage_enabled && length(var.stage_names) > 0 ? var.api_log_enabled ? 0 : length(var.stage_names) : 0
...
resource "aws_api_gateway_stage" "with_log" {
  ...
  xray_tracing_enabled  = length(var.xray_tracing_enabled) > 0 ? element(var.xray_tracing_enabled, count.index) : false
  access_log_settings {
    destination_arn = aws_cloudwatch_log_group.group.*.arn[0]
...
# Module      : Api Gateway account
# Description : Terraform module para configurar o role arn para a conta.
...
# Module     : Api Gateway Usage Plan
#Description : Terraform module to create Api Gateway Usage Plan resource on AWS

resource "aws_api_gateway_usage_plan" "usage-plan" {
  count       = var.usage_plan_count > 0 ? var.usage_plan_count : 0
  name        = element(var.usage_plan_names, count.index)
  description = length(var.usage_plan_descriptions) > 0 ? element(var.usage_plan_descriptions, count.index) : ""

  api_stages {
    api_id = aws_api_gateway_rest_api.default.*.id[0]
    stage  = element(var.stage_names, count.index)
  }
  ...
  throttle_settings {
    burst_limit = element(var.usage_plan_burst_limit, count.index)
    rate_limit  = element(var.usage_plan_rate_limit, count.index)
  }
  depends_on = [
    aws_api_gateway_stage.with_log
  ]
}

resource "aws_api_gateway_usage_plan_key" "main" {
  count         = var.usage_plan_count > 0 && var.key_count > 0 ? var.usage_plan_count : 0
  key_id        = aws_api_gateway_api_key.default.*.id[0]
  key_type      = "API_KEY"
  usage_plan_id = aws_api_gateway_usage_plan.usage-plan.*.id[0]
}

# Module     : Api Gateway Domain Name
#Description : Terraform module to create Api Gateway Domain Name resource on AWS

resource "aws_api_gateway_domain_name" "dns-api" {
  count                    = var.domain_name_enabled ? 1 : 0
  regional_certificate_arn = var.dns_regional_certificate_arn
  domain_name              = var.dns_api
  security_policy          = var.dns_tls_version
  endpoint_configuration {
    types = ["REGIONAL"]
  }
  tags = merge(var.tags,
...
resource "aws_api_gateway_base_path_mapping" "base_path" {
  count       = var.domain_name_enabled ? 1 : 0
  api_id      = aws_api_gateway_rest_api.default.*.id[0]
  domain_name = aws_api_gateway_domain_name.dns-api.*.domain_name[0]
  stage_name  = element(var.stage_names, count.index)
...
resource aws_route53_record record-api {
  count    = var.route53_record_enabled ? 1 : 0
  type     = "A"
  name     = aws_api_gateway_domain_name.dns-api.*.domain_name[0]
  zone_id  = var.route53_zone_id

  alias {
    evaluate_target_health = false
    name                   = aws_api_gateway_domain_name.dns-api.*.regional_domain_name[0]
    zone_id                = aws_api_gateway_domain_name.dns-api.*.regional_zone_id[0]
  }
}
...
resource "aws_cloudwatch_log_group" "group" {
  ...
  name              = "API-Gateway-Execution-Logs_${aws_api_gateway_rest_api.default.*.id[0]}/${element(var.stage_names, count.index)}"
